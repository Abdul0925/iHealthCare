  ...
  <h2 padding>Appointments</h2>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<button class='btn btn-success' onclick='approveAppointment(" . $row["id"] . ")'>Update</button>";
    ...
    <script>
        function approveAppointment(id) {
            ...
            window.location.href = 'update.php?action=approve&id=' + id;
        }

        function ignoreAppointment(id) {
            // You can implement the logic to handle ignoring in PHP
            window.location.href = 'ignore.php?action=ignore&id=' + id;
        }
    </script>
     <footer id="footer">
